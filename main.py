...
def check_xml(bind_xml_after_strip: list[str]) -> list[str]:
    """
    ...
    match bind_xml_after_strip:
        case ['<?xml version="1.0" encoding="UTF-8"?>', '<List>', _, _, _,
              ...
              *_xml_after_match, '</Document>',
              '</List>']:
            print("xml_after_match: ", _xml_after_match)
        case _:
            raise SyntaxError("Формат файла XML не стандартный")
    return _xml_after_match


if __name__ == '__main__':

    xml_after_strip: list[str] = open_and_strip_xml()
    print("xml_after_strip: ", xml_after_strip)

    # список для обработки xml-файла - удаления из него всё до тега <Document> и проверка его на небольшое соответствие
    xml_after_match: list[str] = check_xml(xml_after_strip)
